...
namespace Neurotoxin.ScOut.Mappers
{
    ...
    {
        private readonly IMapper<SyntaxTree, SourceFile> _sourceFileMapper;
        ...
        public ProjectMapper(IMapper<SyntaxTree, SourceFile> sourceFileMapper, ITargetFrameworkMapper targetFrameworkMapper, ISourceFileFiltering rules)
        {
        ...
        {
            var trees = proj.GetCompilationAsync().GetAwaiter().GetResult().SyntaxTrees;
            var sourceFiles = trees.Where(Included).Where(NotExcluded).Select(_sourceFileMapper.Map);
